...
\begin{enumerate}[label=\textbf{Vincolo Integrità \arabic*}:, ref=\textbf{Vincolo Integrità \arabic*}, resume=integrity-constraints, leftmargin=*]
    ...
    \item Ogni corso di laurea deve utilizzare almeno un edificio per svolgere le lezioni
    \label{ic:degree_building_participation} 
\end{enumerate}
...
\begin{itemize}
    ...
    \item \ref{ic:degree_building_participation}: ogni corso di laurea deve utilizzare almento un edifico.
...
L'implementazione del vincolo viene realizzata mediante i seguenti trigger:
\begin{itemize}
    \item Un trigger di tipo \texttt{AFTER INSERT} (Constraint Trigger) applicato alla relazione \texttt{Corso di Laurea}, dichiarato come \texttt{DEFERRABLE INITIALLY IMMEDIATE}, che impone il rollback della transazione al momento del \textit{Commit} (previa istruzione di differimento) qualora non sia presente almeno un'istanza nella relazione \texttt{Corso} che si riferisce al corso di laurea appena inserito.
...
\subsubsection{\ref{ic:degree_building_participation}}

Tale vincolo richiede che ogni corso di laurea utilizzi almeno un edificio per lo svolgimento delle sue lezioni.

L'implementazione del vincolo viene realizzata mediante i seguenti trigger:
\begin{itemize}
    \item Un trigger di tipo \texttt{AFTER INSERT} (Constraint Trigger) applicato alla relazione \texttt{Corso di Laurea}, dichiarato come \texttt{DEFERRABLE INITIALLY IMMEDIATE}, che impone il rollback della transazione al momento del \textit{Commit} (previa istruzione di differimento) qualora non sia presente almeno un'istanza nella relazione \texttt{Edificio} che si riferisce all'corso di laurea appena inserito.
    \item Un trigger di tipo \texttt{BEFORE UPDATE} applicato alla relazione \texttt{Edificio} che impedisce la modifica del riferimento all'corso di laurea nel caso in cui il corso di laurea di provenienza rimanga privo di edifici.
    \item Un trigger di tipo \texttt{BEFORE DELETE} applicato alla relazione \texttt{Edificio} che proibisce la cancellazione di un'istanza qualora essa rappresenti l'ultimo edificio rimasto utilizzato dal corso di laurea di riferimento.
\end{itemize}
...
L'implementazione del vincolo viene realizzata mediante i seguenti trigger:
\begin{itemize}
    \item Un trigger di tipo \texttt{AFTER INSERT} (Constraint Trigger) applicato alla relazione \texttt{Edificio}, dichiarato come \texttt{DEFERRABLE INITIALLY IMMEDIATE}, che impone il rollback della transazione al momento del \textit{Commit} (previa istruzione di differimento) qualora non sia presente almeno un'istanza nella relazione \texttt{Aula} che si riferisce all'edificio appena inserito.
